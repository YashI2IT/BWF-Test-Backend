...
async function createNotice(req, res) {
  try {
    ...
    const authorName = user ? user.name : "Warden";
...
async function updateNotice(req, res) {
  try {
    ...
    if (notice.authorRole !== 'warden') {
      return res.status(403).json({ message: "You can only edit warden notices" });
    }
...
async function deleteNotice(req, res) {
  try {
    ...
    if (notice.authorRole !== 'warden') {
      return res.status(403).json({ message: "You can only delete warden notices" });
    }
